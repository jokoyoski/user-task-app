...
describe('TasksService', () => {
  ...
  describe('getTasksFromApi', () => {
    it('should send a GET request to /tasks', () => {
      service.getTasksFromApi().subscribe();

      const req = httpTestingController.expectOne(
        (request) =>
          request.method === 'GET' && request.url.endsWith('/api/tasks'),
      );
  ...
  describe('searchTask', () => {
    ...
    it('should search task list for a fuzzy match on title', () => {
      service.tasks = [
        generateTask({ title: 'Complete the assignment' }),
        generateTask({ title: 'Send completion report' }),
      ];
      service.searchTask('comp');
      expect(service.tasks.length).toEqual(2);
    });
  });
});
